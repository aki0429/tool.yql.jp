...
function fetchEarthquakeData() {
    $.getJSON("https://api.p2pquake.net/v2/history?codes=551&limit=1&", function(data) {
        ...
        var maxIntText = maxInt_data == 10 ? "1" : maxInt_data == 20 ? "2" : maxInt_data == 30 ? "3" : maxInt_data == 40 ? "4" :
                         ...
                         maxInt_data == 60 ? "6強" : maxInt_data == 70 ? "7" : "";
        ...
        var Depth = data[0]['earthquake']['hypocenter']['depth'] != -1 ?
                    "約"+data[0]['earthquake']['hypocenter']['depth']+"km" : "";
        ...
        var foreignEarthquakeMessage = data[0]['issue']['type'] === "Foreign" ? "遠地地震に関する情報です。" : "地震についての情報です。";
        let foreignEarthquakeMessage_magnitude_depth = data[0]['issue']['type'];
        ...
        if (Name) {
            ...
            Prefls = [...new Set(maxScaleAddrs)].join('、') ;
        }
        // 震度情報の処理
        ...
        if (depth === 0) {
            classification = '浅い'; // 0kmは浅い
        } else {
            classification = depth + " km"; // 数字をそのまま表示
        }
        
        function generateEarthquakeMessage(depth, magnitude, classification, foreignEarthquakeMessage_magnitude_depth) {
            if (foreignEarthquakeMessage_magnitude_depth === "Foreign") {
                // 遠地地震情報がある場合、マグニチュードだけを出力
                return `地震の規模を示すマグニチュードは${magnitude}と推定されています。`;
            }
            ...
            return `震源の深さは${classification}、地震の規模を示すマグニチュードは${magnitude}と推定されています。`;
        }
        function p2pquake_Destination_text(foreignEarthquakeMessage_magnitude_depth) {
            if (foreignEarthquakeMessage_magnitude_depth === "Destination") {
                // 遠地地震情報がある場合、マグニチュードだけを出力
                return `取得先の都合上震源情報には地域ごとの震度データが無いため表示されません。`;
            }   
            return ``;
        }
        ...
        // 最大震度がある場合の処理
        if (maxIntText) {
            info = foreignEarthquakeMessage + "\n" +
                   Time + "頃、" + Prefls + "で最大震度" + maxIntText + "の地震がありました。\n" +
                   tsunamiText + "\n" +
                   generateEarthquakeMessage(depth, Magnitude, classification, foreignEarthquakeMessage_magnitude_depth) + "\n\n"+ p2pquake_Destination_text(foreignEarthquakeMessage_magnitude_depth)+
                   pointsInfo;
        } else {
            info = foreignEarthquakeMessage + "\n" +
                   Time + "頃、" + Prefls + "で地震がありました。\n" +
                   tsunamiText + "\n" +
                   generateEarthquakeMessage(depth, Magnitude, classification, foreignEarthquakeMessage_magnitude_depth) + "\n\n" + p2pquake_Destination_text(foreignEarthquakeMessage_magnitude_depth) +
                   pointsInfo;
        }
