...
for (let i = 1; i <= totalFiles; i++) {
    const filename = `A32-23_${String(i).padStart(2, '0')}.geojson`;
    promises.push(
        fetch(filename)
        .then(response => {
            if (!response.ok) {
            ...
            onEachFeature: function (feature, layer) {
                ...
                if (feature.geometry.type.includes('Polygon') && properties) {
                    layer.on('click', function () {
                        ...
                        infoTitle.textContent = properties.A32_004 || '名称未設定';
                        let detailsHtml = `<p><strong>住所:</strong> ${properties.A32_005 || '情報なし'}</p>`
                                        + `<p><strong>自治体:</strong> ${properties.A32_002 || '情報なし'}</p>`
                                        + `<p><strong>ID:</strong> ${properties.A32_001 || '情報なし'}</p>`;
