...
async function showEarthquakeInfo(data) {
    try {
        ...
        eqDetails.textContent = `震源地：${eq.hypocenter.name}　マグニチュード：M${magnitude}\n震源の深さ：${depth} km　${tsunami}`;
        ...
        for (let i = 0; i < intensityInfoText.length; i += 2) {
            const linesToShow = intensityInfoText.slice(i, i + 2).join('\n');
